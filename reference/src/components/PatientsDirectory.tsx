...
import {
  ...
  FlaskConical, Camera, Mic, MicOff, Syringe, Ban, MapPin, Pill
} from "lucide-react";
...
export default function PatientsDirectory({
  ...
  const [activeWorkspaceTab, setActiveWorkspaceTab] = useState<"chart_review" | "synopsis" | "assessment" | "plan" | "orders" | "medications">("chart_review");
  ...
  const handleCreateNewBlankNote = () => {
    ...
    setActiveNoteBp("");
    setActiveNoteHr("");
    // Default the location from today's appointment for this patient, if one
    // exists and has a location set -- otherwise leave it for the doctor to
    // pick manually (e.g. an unscheduled walk-in or home visit).
    const todayStr = new Date().toDateString();
    const todaysAppt = (patientAppointments || []).find((apt: any) =>
      apt.location_id && new Date(apt.schedule_time).toDateString() === todayStr
    );
  ...
  const insertTestAtCursor = (testHtml: string, testName: string) => {
    ...
    document.execCommand('insertHTML', false, testHtml);
    setActiveNoteContent(editorRef.current.innerHTML);
      ...
      if (data && data.length > 0) {
        ...
        const mappedData: Patient[] = data.map((item: any) => {
          ...
          return {
            ...
            phone,
            email: item.email || "",
            history: item.history || "None declared.",
  ...
  const handleCreatePatient = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        history: newPatientHistory.trim() || null,
      ...
      if (resultData) {
        ...
        const created: Patient = {
          ...
          history: resultData.history || "None declared.",
  ...
  const handleSaveEditPatient = async (e: React.FormEvent) => {
    ...
    const payload: any = {
      ...
      history: (editForm.history || "").trim() || null,
    ...
    try {
      const { error } = await supabase.from("patients").update(payload).eq("mrn", Number(selectedPatient.mrn ?? selectedPatient.id));
      if (error) throw error;
      const updated: Patient = {
        ...
        history: payload.history || "None declared.",
            ...
            <>
              ...
              <div className="absolute left-0 mt-2 w-80 bg-white border border-slate-200 rounded-xl shadow-xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
                ...
                <div className="flex items-center gap-3">
                  ...
                  >
                    <UserPlus size={12} /> Admit Patient
                  </button>
                  {selectedPatient && (
                  <button
                          ...
                          {isNoteSigned ? (
                            ...
                            <>
                              ...
                              <button 
                                ...
                                disabled={isNoteSaving}
                                className="bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold px-3 py-1.5 rounded text-[10px] flex items-center gap-1 cursor-pointer shadow-xs"
                              >
